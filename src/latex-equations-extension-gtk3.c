...
static void m_msg_composer_extension_add_menu_item(MMsgComposerExtension *ext, EMsgComposer         *composer)
{
	...
	/*
	 *      * Add menu item:
	 *           * Message Composer → Format → Convert LaTeX equations
	 *                */
	...
	if (error) {
		g_warning ("Failed to add Format menu item: %s", error->message);
		g_clear_error (&error);
	}
